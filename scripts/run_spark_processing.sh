...
COUNT=$(docker exec nyc-taxi-postgres psql -U taxiuser -d nyc_taxi -t -c "SELECT COUNT(*) FROM raw.trip_data;" 2>/dev/null | xargs)
...
if ! docker exec nyc-taxi-spark-master sh -c "mkdir -p /tmp/jars && curl -L -o /tmp/jars/postgresql-42.6.0.jar https://jdbc.postgresql.org/download/postgresql-42.6.0.jar" 2>/dev/null; then
...
# Run the Spark job with local JAR (no package resolution needed)
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    PROC_COUNT=$(docker exec nyc-taxi-postgres psql -U taxiuser -d nyc_taxi -t -c "SELECT COUNT(*) FROM processed.trip_features;" 2>/dev/null | xargs)
    HOUR_COUNT=$(docker exec nyc-taxi-postgres psql -U taxiuser -d nyc_taxi -t -c "SELECT COUNT(*) FROM aggregated.hourly_stats;" 2>/dev/null | xargs)
    DAILY_COUNT=$(docker exec nyc-taxi-postgres psql -U taxiuser -d nyc_taxi -t -c "SELECT COUNT(*) FROM aggregated.daily_stats;" 2>/dev/null | xargs)
